...
def reshape_classifier_output(model, n=1000):
    ...
    name, m = list((model.model if hasattr(model, "model") else model).named_children())[-1]  # last Model
...
def model_info(model, verbose=False, imgsz=640):
    ...
    name = Path(model.yaml_file).stem.replace("yolov5w", "YOLOv5") if hasattr(model, "yaml_file") else "Model"
...
def smart_hub_load(repo="ultralytics/yolov5w", model="yolov5s", **kwargs):
